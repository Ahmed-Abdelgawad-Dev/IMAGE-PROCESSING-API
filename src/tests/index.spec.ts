...
describe('Testing endpoints:', () => {
    ...
    it('Resizing endpoint works.', async () => {
        const resp = await request.get(
            '/api/images/resize_image/?width=500&height=500&filename=pic1',
        )
        expect(resp.status).toBe(200)
    })
    ...
    it('Cached image functionality works.', async () => {
        const resp = await request.get(
            '/api/images/resize_image/?width=200&height=200&filename=pic2',
        )
        expect(
            fs.existsSync(`${IMAGES_OUTPUT_PATH}/pic2_200_200.jpg`),
        ).toBeTruthy()
        expect(resp.status).toBe(200)
    })
...
describe('Testing Processing Image Functionalities:', () => {
    ...
    it('Testing image does not exist.', () => {
        expect(
            fs.existsSync(`${IMAGES_OUTPUT_PATH}/pic2_333_333.jpg`),
        ).toBeFalsy()
    })
})
